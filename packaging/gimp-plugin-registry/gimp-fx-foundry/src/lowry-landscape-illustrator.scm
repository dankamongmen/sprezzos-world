...
(define (script-fu-Land_illust  img drawable blur-rad merge-flag sharpen-flag strength )
   ...
   (gimp-drawable-set-name darken-layer _"Darken Only layer")
   ...
   (if (equal? merge-flag TRUE)
       ...
       (gimp-drawable-set-name merged-layer _"Result of Landscape Illustrator")
...
(script-fu-register "script-fu-Land_illust"
      _"_Landscape Illustrator"
      _"Similar to Landscape Painter, but with a bit of a sketch effect as well. Oversharpen, then add Darken Only layer and blur it"
      "Mark Lowry"
      ...
      SF-VALUE "_Blur radius?"   "15"
      SF-TOGGLE _"Merge Layers?"  FALSE
      SF-TOGGLE _"Sharpen First?" TRUE
      SF-ADJUSTMENT _"Sharpening strength?"  '(5 0 10 1 10 0 0)
 )
