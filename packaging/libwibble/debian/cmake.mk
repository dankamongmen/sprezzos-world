...
$(DEB_BUILDDIR)/CMakeCache.txt:
	cd $(DEB_BUILDDIR) && cmake $(CURDIR)/$(DEB_SRCDIR) -DCMAKE_INSTALL_PREFIX="$(DEB_CMAKE_PREFIX)" $(DEB_CMAKE_EXTRA_FLAGS) -DCMAKE_CXX_FLAGS="$(CXXFLAGS)" -DCMAKE_C_FLAGS="$(CFLAGS)" -DCMAKE_VERBOSE_MAKEFILE=ON
	mkdir -p $(DEB_DESTDIR)
